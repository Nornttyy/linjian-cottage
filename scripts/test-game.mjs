...
try {
 ...
 test('harvesting requires the correct tool and proximity; three hits yield six wood',()=>{const f=fixture(),r=world.RESOURCE_MAP.get('247:315');assert(r);f.p.x=r.x-.6;f.p.y=r.y+.5;assert.equal(command(f,{type:'attack',tool:'pick',target:r.id}),'使用斧头');assert.equal(f.state.resourceHp[r.id],undefined);for(let i=0;i<3;i++)command(f,{type:'attack',tool:'axe',target:r.id});assert.equal(f.p.inventory.wood,6);assert.equal(f.state.depleted[r.id],true);command(f,{type:'attack',tool:'axe',target:r.id});assert(f.p.inventory.wood>=7&&f.p.inventory.wood<=9);assert(f.state.removedStumps[r.id]);const gained=f.p.inventory.wood;command(f,{type:'attack',tool:'axe',target:r.id});assert.equal(f.p.inventory.wood,gained);const far=world.RESOURCES.find(x=>x.kind==='tree'&&x.x<50);assert.equal(command(f,{type:'attack',tool:'axe',target:far.id}),'距离太远');});
 ...
 test('death waits for respawn and returns the player to camp without deleting materials',()=>{const f=fixture();f.p.hp=0;f.p.x=350;f.p.inventory.wood=17;sim.tickWorld(f.state,f.now+100);assert(f.p.death);assert.equal(f.p.x,350);assert.equal(f.p.hp,0);assert.equal(sim.respawnPlayer(f.state,f.p,f.now+1900),true);assert.equal(f.p.x,world.SPAWN.x);assert.equal(f.p.hp,75);assert.equal(f.p.inventory.wood,17);});
