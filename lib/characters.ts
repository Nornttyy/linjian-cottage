...
export type Character={id:string;name:string;createdAt:number};
...
export function readCharacters(raw:string|null):CharacterList{
    try{
        ...
        return {version:1,active:characters.some((p:Character)=>p.id===value.active)?value.active:'',characters};
    }catch{return empty();}
}
...
export function createCharacter(name:string){
    ...
    const hero={id:crypto.randomUUID(),name:clean,createdAt:Date.now()};list.characters.push(hero);list.active=hero.id;saveList(list);return hero;
}
