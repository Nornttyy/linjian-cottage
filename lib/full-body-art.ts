...
function hairRegion(p:Pixels,reference:Region|undefined,tool:Uint8Array,family:BodyFamily){
    ...
    const hair=(i:number)=>{const k=i*4,x=i%w,y=i/w|0;return Math.abs(x-cx)<=20&&Math.abs(y-cy)<=18&&!(tool[i]<=3&&y<cy-3)&&!(family==='mushroom'&&d[k]>160&&d[k]>d[k+1]*3)&&d[k+3]>=128&&hairColour(d[k],d[k+1],d[k+2]);};
...
export function completeBodyParts(p:Pixels,reference:Parts,family:BodyFamily,frame:Sprite,body:Appearance['body']='female'):Parts{
    ...
    // the face before finding components, rather than rejecting the whole
    // joined region and mistaking a tiny highlight for the character's face.
    const faces=!frame.includes('-up-')?regions(p,i=>{
        const k=i*4,r=d[k],g=d[k+1],b=d[k+2];
        return Math.abs(i%w-fx)<13&&Math.abs((i/w|0)-fy)<12&&d[k+3]>=128&&r>195&&g>135&&b>65&&r>=g*.985&&g>=b*.98&&r-g<65;
    }).filter(b=>b.points.length>=8):[];
    ...
    // White highlights belong to metal only beside the actual blue tool. A
    // broad distance from an old tool pose also reaches skin and cream cloth.
    ...
    for(let i=0;i<w*p.height;i++){
        ...
        if(inFace){
            ...
            if(eyeMarks.has(i)||r<130&&g<105&&b<100&&brightNeighbors>=2){assign('eyes',i);continue;}
        ...
        if(inHead){
            ...
            if(skinPixels.has(i)||inFace&&(paleSkin||r>225&&g>210&&b>170)||backNeck){assign('skin',i);continue;}
        ...
        if(family==='swim'){
            ...
            if(warm&&distance.shoes[i]<=8&&distance.shoes[i]<distance.skin[i]+2){assign('shoes',i);continue;}
...
export function registerCompleteBody(raw:HTMLCanvasElement,base:HTMLCanvasElement,frame:Sprite,body:Appearance['body'],family:BodyFamily,scale:number){
    ...
    const rodTip=completeRodTip(p,frame,body),parts=completeBodyParts(p,reference,family,frame,body);
...
export function completeRodTip(p:Pixels,frame:Sprite,body:Appearance['body']):readonly [number,number]|undefined{
    const rod=authoredFishingRod(p,frame,body)??fishingRod(frame,body);if(!rod)return;
