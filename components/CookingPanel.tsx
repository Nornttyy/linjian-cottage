...
import {COOK_METHODS,DISHES,DISH_IDS,INGREDIENTS,INGREDIENT_IDS,MAX_COOKING_INGREDIENTS,ingredientCounts,recipeExample,resolveRecipe,validateSelection,type CookMethod,type IngredientId} from '@/lib/cooking';
...
type Props={player:Player;busy:boolean;supported:boolean;connected:boolean;onClose:()=>void;onCook:(method:CookMethod,ingredients:IngredientId[])=>boolean;onPantry:(offer:PantryOffer)=>boolean};
export default function CookingPanel({player,busy,supported,connected,onClose,onCook,onPantry}:Props){
    ...
    const near=nearCampfire(player),disabled=busy||!connected||!near||!supported;
    ...
    const chooseRecipe=(id:typeof DISH_IDS[number])=>{
        const example=recipeExample(id);setMethod(DISHES[id].method);setIngredients(example);setTab('cook');
    };
    return <div className="modal-backdrop cooking-backdrop" onClick={onClose}><section className="cooking-panel" role="dialog" aria-modal="true" aria-labelledby="cooking-title" onClick={event=>event.stopPropagation()} onKeyDown={event=>{
        ...
        <nav className="kitchen-tabs" aria-label="厨房页面">{([['cook','料理'],['recipes','配方'],['pantry','食材补给']] as const).map(([id,name])=><button key={id} onClick={()=>setTab(id)} aria-pressed={tab===id}>{name}</button>)}</nav>
        ...
        {tab==='cook'&&<>
            ...
            <section className="recipe-preview" aria-live="polite" aria-label="料理预览">
                <div className="recipe-dish">{recipe&&<Icon name={recipe.id} size={48}/>}<div><strong>{recipe?DISHES[recipe.id].name:ingredients.length?'换个组合试试':'选择食材和做法'}</strong><span>{recipe?`制作 ${recipe.quantity} 份 · 生命+${recipe.hp} · 体力+${recipe.stamina}`:ingredients.length?'可在配方中查看搭配':'同一种食材可以放多份'}</span></div></div>
        ...
        {tab==='recipes'&&<div className="recipe-book">{DISH_IDS.filter(id=>DISHES[id].method===method).map(id=>{
            const example=recipeExample(id),counts=ingredientCounts(example),owned=validateSelection(example,inventory).ok;
            return <button className="recipe-card" key={id} disabled={disabled} onClick={()=>chooseRecipe(id)} aria-label={'选择'+DISHES[id].name+'配方'}><Icon name={id} size={40}/><div><strong>{DISHES[id].name}</strong><span>{Object.entries(counts).map(([item,count])=>INGREDIENTS[item as IngredientId].name+'×'+count).join(' · ')}</span><small>{owned?'材料齐全':'可先选配方，再准备食材'}</small></div></button>;
        })}</div>}
        {tab==='pantry'&&<>
        ...
        <footer className="kitchen-footer" aria-live="polite">{!connected?'连接中断，正在重连':!near?'请回到营火旁继续制作':busy?'正在准备料理…':result?`${DISHES[result.dish].name} ×${result.quantity} 已放入背包`:'关闭窗口后，可在快捷栏食用料理'}</footer>
        </>}
    </section></div>;
}
