...
export default function SiteEntry({children}:{children:ReactNode}){
    ...
    const status=failed?'素材加载失败':ready?'准备就绪':loading.phase==='atlas'||loading.phase==='ready'?'正在整理素材…':'正在加载素材…';
    return <section className={'site-entry'+(ready?' entry-ready':'')} aria-label="正在准备林间小筑" aria-busy={!ready&&!failed}>
        ...
        <div className="entry-content"><div className="entry-lantern" aria-hidden="true"/><h1 className="entry-title">林间小筑</h1>
            ...
            <div className="entry-progress"><span>{ready?'准备就绪':'加载进度'}</span><span>{loading.loaded} / {loading.total} 份素材</span></div>
